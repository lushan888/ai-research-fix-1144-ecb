"""
Fix for Issue #1440 — Log Injection → Log Forging → SIEM Poisoning ($120)
================================================================================

Vulnerability
-------------
User-controlled input (username, User-Agent, etc.) is written directly to log
files without sanitization. Attackers inject CRLF (\\r\\n) sequences to forge
fake log entries, poison SIEM detection logic, trigger false alerts, or hide
malicious activity.

Fix Strategy
------------
1. Strip/escape all \\r and \\n characters from log inputs (CRLF removal)
2. Use structured JSON logging format (prevents log forging entirely)
3. Apply log schema validation on every field
4. Reject entries exceeding safe length limits
5. Provide a CRLF-safe legacy logger for backward compatibility

Acceptance Criteria
-------------------
- [x] \\r \\n are removed/escaped from all log inputs
- [x] Structured logging (JSON format) is used
- [x] Log schema validation is introduced
"""
        ...
        try:
            ...
            if len(serialized) > self.config.max_entry_length:
                return (
                    False,
                    sanitized,
                    f"Serialized entry exceeds {self.config.max_entry_length} bytes",
                )
...
def test_schema_validation():
    ...
    assert ok, err
    ok, err = v.validate(
        {"timestamp": "2026-01-01T00:00:00Z", "level": "INVALID", "message": "hello"}
    )
    assert not ok
...
def test_structured_json_logging():
    ...
    line = logger.log(
        "INFO", "user logged in", username="admin\nbad", user_agent="Mozilla\r\nInjected"
    )
...
def test_legacy_crlf_safe_logger():
    ...
    # only trailing newline is allowed
    assert '\n' not in line.replace('\n', '', 1)
    print("PASS: Legacy CRLF-safe logger works")


def test_siem_poisoning_vectors():
    """Test that common SIEM poisoning vectors are blocked."""
    logger = SecureJSONLogger()

    # Attack: inject fake ERROR entry
    line = logger.log(
        "INFO",
        "normal login",
    ...
    parsed = json.loads(line)
    assert '\n' not in parsed['username']
    print("PASS: SIEM fake entry injection blocked")

    # Attack: inject fake CRITICAL level
    line = logger.log(
        "WARNING",
        "rate limit hit",
    ...
    parsed = json.loads(line)
    assert '\r' not in parsed['username']
    print("PASS: SIEM fake level injection blocked")

    # Attack: inject multiple log entries via User-Agent
    line = logger.log_access(
        "10.0.0.1",
        "GET",
        "/",
    ...
    parsed = json.loads(line)
    assert '\r' not in parsed.get('user_agent', '')
    print("PASS: SIEM multi-entry injection blocked")
